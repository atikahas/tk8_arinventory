    ...
    <div class="breadcrumb-wrapper">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb p-0">
                ...
                <li class="breadcrumb-item" aria-current="page">edit items</li>
            </ol>
        </nav>
    </div> 
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('items.update', $item->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3 form-group">
                            ...
                            <textarea class="form-control" name="item_description" rows="3" placeholder="Enter item description">{{ $item->item_description }}</textarea>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-6 mb-3 form-group">
                                ...
                                <input type="number" min="0" class="form-control" name="current_stock" value="{{ $item->current_stock }}" required>
                            </div>
                            <div class="col-6 mb-3 form-group">
                                <label>Initial Stock</label>
                                <input type="number" min="0" class="form-control" name="initial_stock" value="{{ $item->initial_stock }}" required>
                            </div>
                        </div>
...
@section('scriptfooter')
    <script>
        $(document).ready(function() {
            ...
            getSubCategory("{{$_GET['category_id']}}");
            @endif
        }); 
        ...
        function getCategory() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    @if(isset($_GET['category_id']))
                    $("select[name=category_id]").val("{{$_GET['category_id']}}");
                    @endif
                    }
            });
        }
        ...
        function getSubCategory(category_id) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("select[name=subcategory_id]").val("{{$_GET['subcategory_id']}}");
                    @endif
                    }
            });
        }
        ...
        function getLocation() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("select[name=location_id]").val("{{$_GET['location_id']}}");
                    @endif
                    }
            });
        }
